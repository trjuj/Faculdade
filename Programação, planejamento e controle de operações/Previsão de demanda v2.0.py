...
for i, model_predictions in enumerate([future_preds_knn, future_preds_rf]):
    plt.subplot(1, 2, i + 1)
    plt.scatter(dates, y, color='blue', alpha=0.7)
    future_dates_num = mdates.date2num(future_dates)  # Convertendo futuras datas para formato numérico
    plt.scatter(future_dates_num, model_predictions, color='green', label='Previsões Futuras')
